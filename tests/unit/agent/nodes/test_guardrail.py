...
from backend.agent.nodes.guardrail import GuardrailDecision, guardrail
...
async def test_safe_request_sets_is_safe_true() -> None:
    with patch("backend.agent.nodes.guardrail.EngineFactory.create_default", return_value=_mock_engine(True)):
...
async def test_unsafe_request_sets_is_safe_false() -> None:
    with patch("backend.agent.nodes.guardrail.EngineFactory.create_default", return_value=_mock_engine(False, "injection attempt")):
...
async def test_empty_message_content_marks_unsafe_without_calling_engine() -> None:
    with patch("backend.agent.nodes.guardrail.EngineFactory.create_default") as mock_factory:
...
async def test_classifies_last_message_in_list() -> None:
    ...
    with patch("backend.agent.nodes.guardrail.EngineFactory.create_default", return_value=engine):
        state = {
            "messages": [
    ...
    _, call_kwargs = engine.generate_structured.call_args
